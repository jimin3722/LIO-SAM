...
{   
    ...
    // MultiThreadedExecutor는 여러 개의 스레드를 사용하여 노드의 콜백 함수를 병렬로 실행할 수 있습니다. 
    // 이를 통해 CPU 코어를 효율적으로 활용하여 전체 시스템 성능을 향상시킬 수 있습니다.
    // e.spin(); 을 호출하면 실행자가 실행됩니다.
    rclcpp::executors::MultiThreadedExecutor e;

    // 스마트 포인터 생성 함수입니다. 이 함수는 std::shared_ptr<>를 생성하고 초기화하는 데 사용됩니다. 
    // std::shared_ptr<>는 메모리 관리를 자동으로 처리하는 스마트 포인터 클래스입니다.
    // std::shared_ptr< 클래스 이름 > ptr = std::make_shared< 클래스 이름 >();
